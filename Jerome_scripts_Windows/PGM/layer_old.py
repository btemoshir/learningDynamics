...
class Layer():
    def __init__(self, N = 100, nature = 'Bernoulli', position = 'visible', n_c = 1, gauge = 'zerosum', random_state = None,zero_field = False):
        ...
        if self.nature in ['Gaussian','ReLU','ReLU+']:
            self.type=np.float
        else:
            self.type = np.int
...
#%%
    ...
    def transform(self,psi):
        ...
        if self.nature == 'Gaussian':
            ...
            #return np.maximum(psi - self.theta_plus[np.newaxis,:],0)/self.a[np.newaxis,:] + np.minimum(psi + self.theta_minus[np.newaxis,:],0)/self.a[np.newaxis,:]
        elif self.nature == 'ReLU+':
    ...
    def random_init_config(self,n_samples,N_PT=1):
        ...
        elif self.nature in ['Gaussian','ReLU']:
            if N_PT>1:
