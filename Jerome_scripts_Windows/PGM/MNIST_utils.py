...
def augment_dataset(data,factor = 1, eps = 0.2, mu = None,reshape = True):
    if mu is None:
        mu = data.mean(0)
    big_mu = np.repeat(mu[np.newaxis,:] , data.shape[0],0)
    liste = [data]
    for _ in range(factor):
#        liste.append( np.logical_xor(data, np.random.rand(data.shape[0], data.shape[1]) < eps ) )        
        index = (np.random.random(size= data.shape) < eps)
        data2 = data.copy()
        data2[index] = np.random.random(size=data2[index].shape) < big_mu[index]
        liste.append(data2)
    if reshape:
        return np.swapaxes(np.array(liste),0,1).reshape([ (1+factor) * data.shape[0], data.shape[1] ])
    else:
        return liste
...
def show_samples(PGM, show = True, Nchains = 100, Nthermalize =500, N_PT = 10, batches = 10):
    size = int(np.sqrt(PGM.n_visibles))        
    _, datah = PGM.gen_data(Nchains = Nchains,Lchains=1,Nthermalize=Nthermalize,N_PT=N_PT,batches=batches)
