...
import pgm
import utilities
from utilities import check_random_state,gen_even_slices,logsumexp,log_logistic
import cy_utilities
...
class BM(pgm.PGM):
    ...
    def compute_all_moments(self):
       if self.nature == 'Bernoulli':
           ...
           string = ','.join(['[-1,1]' for _ in range(self.N)])
           exec('iter_configurations=itertools.product(%s)'%string)
       elif self.nature =='Potts':
           tmp = '[' + ','.join( ['%s'%a for a in range(self.n_c)] ) + ']'
           string = ','.join([tmp for _ in range(self.N)])
           exec('iter_configurations=itertools.product(%s)'%string)
       else:
           print 'no supported'
    ...
    def fit(self,data, batch_size=100, nchains = 100, learning_rate = None, extra_params = None, init='independent', optimizer='SGD',N_PT = 1, N_MC = 1, n_iter = 10,
            lr_decay = True,lr_final=None,decay_after = 0.5,l1 = 0, l1b = 0, l1c=0, l2 = 0,no_fields = False, batch_norm = False,
        ...
        covariance = utilities.average_product(data,data, c1 = self.n_c, c2 = self.n_c,weights=weights)
        if pseudo_count>0:
            p = data.shape[0]/float(data.shape[0] + pseudo_count)
            covariance = p**2 * covariance + p*(1-p) * (mean[np.newaxis,:,np.newaxis,:] * mean[:,np.newaxis,:,np.newaxis])/self.n_c + (1-p)**2/self.n_c**2
        ...
        if self.nature == 'Potts':
            (self.fantasy_x,self.fantasy_fields_eff) = self.layer.sample_from_inputs(np.zeros([self.N_PT*self.nchains,self.N,self.n_c]) ,beta=0 )
        else:
            (self.fantasy_x,self.fantasy_fields_eff) = self.layer.sample_from_inputs(np.zeros([self.N_PT*self.nchains,self.N]) ,beta=0 )
        if self.N_PT > 1:
            self.fantasy_x = self.fantasy_x.reshape([self.N_PT,self.nchains,self.N])
            if self.nature == 'Potts':
                self.fantasy_fields_eff = self.fantasy_fields_eff.reshape([self.N_PT,self.nchains,self.N,self.n_c])
            else:
                self.fantasy_fields_eff = self.fantasy_fields_eff.reshape([self.N_PT,self.nchains,self.N])
        ...
        for epoch in xrange(1,n_iter+1):
            ...
            for _ in xrange(iter_per_epoch):
    ...
    def minibatch_fit(self,mean,covariance): 
        ...
        for _ in range(self.N_MC):
            if self.N_PT>1:
                (self.fantasy_x,self.fantasy_fields_eff),self.fantasy_E = self.markov_step_PT2((self.fantasy_x,self.fantasy_fields_eff),self.fantasy_E)
                (self.fantasy_x,self.fantasy_fields_eff),self.fantasy_E = self.exchange_step_PT((self.fantasy_x,self.fantasy_fields_eff),self.fantasy_E,record_acceptance=self.record_acceptance,compute_energy=False)
...
#                (self.fantasy_x,self.fantasy_fields_eff),self.fantasy_E= self.exchange_step_PT((self.fantasy_x,self.fantasy_fields_eff),self.fantasy_E,record_acceptance=self.record_acceptance)
                ...
                (self.fantasy_x,self.fantasy_fields_eff) = self.markov_step((self.fantasy_x,self.fantasy_fields_eff) )
        ...
        self.gradient['layer'] = self.layer.internal_gradients( (mean,covariance),X_neg , l1 = self.l1, l2 = self.l2,value='mean')
        ...
        for internal_param,gradient in self.gradient['layer'].items():
                ...
                if self.optimizer == 'SGD':
                    ...
                    self.previous_update['layer'][internal_param] =  (1- self.momentum) * self.learning_rate * gradient + self.momentum * self.previous_update['layer'][internal_param]
                    new = current + self.previous_update['layer'][internal_param]
                elif self.optimizer == 'ADAM':
                    self.gradient_moment1['layer'][internal_param] = (1- self.beta1) * gradient + self.beta1 * self.gradient_moment1['layer'][internal_param]
                    self.gradient_moment2['layer'][internal_param] = (1- self.beta2) * gradient**2 + self.beta2 * self.gradient_moment2['layer'][internal_param]
                    
                    new = current + self.learning_rate * (self.gradient_moment1['layer'][internal_param]/(1-self.beta1**self.count_updates)) /(self.epsilon + np.sqrt( self.gradient_moment2['layer'][internal_param]/(1-self.beta2**self.count_updates ) ) )
    ...
    def initialize_gradient_dictionary(self):
        out = {}
        out['layer'] = self.layer.internal_gradients(np.zeros([1,self.N],dtype=int), np.zeros([1,self.N],dtype=int))
        return out
