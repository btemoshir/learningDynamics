...
def get_cross_derivatives_dReLU(V_pos,psi_pos, hlayer, n_cv,weights=None):
    ...
    psi_plus = ( -np.sqrt(1+eta) * (psi-b) + theta/np.sqrt(1+eta) )/np.sqrt(a)
    psi_minus = ( np.sqrt(1-eta) * (psi-b) + theta/np.sqrt(1-eta) )/np.sqrt(a)
    ...
    e = (psi-b) * (1+eta * (p_plus-p_minus) ) - theta * (p_plus-p_minus) + 2*eta*np.sqrt(a)/Z
    v = eta * (p_plus-p_minus) + p_plus * p_minus * (2*theta/np.sqrt(a)- 2 * eta * (psi-b)/np.sqrt(a)) * (2*theta/np.sqrt(a)- 2 * eta * (psi-b)/np.sqrt(a) - np.sqrt(1+eta)/Phi_plus - np.sqrt(1-eta)/Phi_minus)- 2 * eta*e/(np.sqrt(a)*Z)
    ...
    dpsi_plus_deta = -1.0/(2*np.sqrt(a*(1+eta))) * ( (psi-b) + theta/(1+eta) )
    dpsi_minus_deta = -1.0/(2*np.sqrt(a*(1-eta))) * ( (psi-b) - theta/(1-eta) )
    ...
    de_dtheta = -(p_plus-p_minus)  + 2*((psi-b) * eta - theta) * dp_plus_dtheta - 2*eta*np.sqrt(a)/Z * dlogZ_dtheta
    de_deta = (psi-b) * (p_plus-p_minus) + 2*( (psi-b)*eta-theta ) * dp_plus_deta + 2*np.sqrt(a)/Z - 2*eta*np.sqrt(a)/Z * dlogZ_deta
