...
TODO :
    beta < 1 code for Gaussian, ReLU, ReLU+ (decide !)
    block sampling for PT
    No internal_couplings for now
...
class Layer():
    ...
    def mean_from_inputs(self,psi, beta = 1):
        if self.nature == 'Bernoulli':
            ...
            if beta == 1:
                ...
                return mean_pos * p_pos + mean_neg * p_neg
            else:
            ...
            etg_plus = erf_times_gauss((-psi + theta_plus[np.newaxis,:])/np.sqrt(a_plus[np.newaxis,:])) 
            etg_minus = erf_times_gauss((psi + theta_minus[np.newaxis,:])/np.sqrt(a_minus[np.newaxis,:]))
            p_pos = 1/(1+ (etg_minus/np.sqrt(a_minus[np.newaxis,:]))/(etg_plus/np.sqrt(a_plus[np.newaxis,:])) )
            p_neg = 1- p_pos
            mean_neg = (psi + theta_minus[np.newaxis,:])/a_minus[np.newaxis,:] - 1/(etg_minus*np.sqrt(a_minus[np.newaxis,:]) )
            mean_pos = (psi - theta_plus[np.newaxis,:])/a_plus[np.newaxis,:] + 1/(etg_plus * np.sqrt(a_plus[np.newaxis,:]) )
            return mean_pos * p_pos + mean_neg * p_neg          
    ...
    def mean2_from_inputs(self,psi, beta = 1):
        if self.nature in ['Bernoulli','Potts']:
            ...
            return (p_pos * mean2_pos + p_neg * mean2_neg)
            ...
            p_pos = 1/(1+ (etg_minus/np.sqrt(a_minus[np.newaxis,:]))/(etg_plus/np.sqrt(a_plus[np.newaxis,:])) )
            p_neg = 1- p_pos
            mean2_pos = 1/a_plus[np.newaxis,:] * (1 +     ((psi - theta_plus[np.newaxis,:])/np.sqrt(a_plus[np.newaxis,:]))**2  -  ((-psi + theta_plus[np.newaxis,:])/np.sqrt(a_plus[np.newaxis,:]))/etg_plus )
            mean2_neg = 1/a_minus[np.newaxis,:] * (1 +     ((psi + theta_minus[np.newaxis,:])/np.sqrt(a_minus[np.newaxis,:]))**2  -  ((psi + theta_minus[np.newaxis,:])/np.sqrt(a_minus[np.newaxis,:]))/etg_minus )
            
            return mean2_pos * p_pos + mean2_neg * p_neg         
    ...
    def mean12_pm_from_inputs(self,psi, beta = 1):
        if self.nature =='dReLU':
            ...
            mean_pos = (psi - theta_plus[np.newaxis,:])/a_plus[np.newaxis,:] + 1/(etg_plus * np.sqrt(a_plus[np.newaxis,:]) )            
            mean_neg = (psi + theta_minus[np.newaxis,:])/a_minus[np.newaxis,:] - 1/(etg_minus*np.sqrt(a_minus[np.newaxis,:]) )
            return (p_pos* mean_pos, p_neg * mean_neg, p_pos * mean2_pos, p_neg * mean2_neg)
    ...
    def var_from_inputs(self,psi,beta =1):
        if self.nature in ['Bernoulli','Potts']:
            ...
            if beta == 1:
                ...
                mean2_pos = 1/self.a[np.newaxis,:] * (1 +     ((psi - self.theta_plus[np.newaxis,:])/np.sqrt(self.a[np.newaxis,:]))**2  -  ((-psi + self.theta_plus[np.newaxis,:])/np.sqrt(self.a[np.newaxis,:]))/etg_plus )
                mean2_neg = 1/self.a[np.newaxis,:] * (1 +     ((psi + self.theta_minus[np.newaxis,:])/np.sqrt(self.a[np.newaxis,:]))**2  -  ((psi + self.theta_minus[np.newaxis,:])/np.sqrt(self.a[np.newaxis,:]))/etg_minus )
                return (p_pos * mean2_pos + p_neg * mean2_neg) - (p_pos * mean_pos + p_neg * mean_neg)**2
            else:
                etg_plus = erf_times_gauss((-beta*psi + (beta*self.theta_plus + (1-beta) * self.theta_plus0)[np.newaxis,:])/np.sqrt( beta*self.a + (1-beta)* self.a0)[np.newaxis,:])
                etg_minus = erf_times_gauss((beta*psi + (beta*self.theta_minus+(1-beta) * self.theta_minus0)[np.newaxis,:])/np.sqrt( beta*self.a + (1-beta)* self.a0)[np.newaxis,:])
                p_pos = 1/(1+ etg_minus/etg_plus)
    ...
    def sample_from_inputs(self,psi,beta=1,previous=(None,None)):
        if self.nature == 'Bernoulli':
            ...
            if beta ==1:
                cum_probas = cumulative_probabilities(psi + self.fields[np.newaxis,:,:])
            else:
                cum_probas = cumulative_probabilities(beta * (psi + self.fields[np.newaxis,:,:]) + (1-beta)*  self.fields0[np.newaxis,:,:])
            ...
            if beta ==1:
                ...
                is_pos = self.random_state.random_sample(size=psi.shape) < p_pos
                rmax = 0 * p_pos
                rmin = 0 * p_pos
            ...
            rmax = 0 * p_pos
            rmin = 0 * p_pos
            rmin[is_pos] = erf( ((theta_plus[np.newaxis,:] - psi)/np.sqrt(a_plus))[is_pos]/np.sqrt(2) )
            rmax[is_pos] = 1
            rmin[~is_pos] = -1
            rmax[~is_pos] = erf( -((psi + theta_minus[np.newaxis,:])/np.sqrt(a_minus))[~is_pos]/np.sqrt(2) )

            h = np.zeros(psi.shape)
            tmp = (rmax - rmin > 1e-14)
            h = np.sqrt(2) * erfinv(rmin + (rmax - rmin) * self.random_state.random_sample(size = h.shape)   )
            h[is_pos]/= np.sqrt(a_plus[np.newaxis,:]).repeat(psi.shape[0],axis=0)[is_pos]            
            h[is_pos] += psi[is_pos]/a_plus[np.newaxis,:].repeat(psi.shape[0],axis=0)[is_pos]
            h[is_pos] -= (theta_plus/a_plus)[np.newaxis,:].repeat(psi.shape[0],axis=0)[is_pos]

            h[~is_pos]/= np.sqrt(a_minus[np.newaxis,:]).repeat(psi.shape[0],axis=0)[~is_pos]                    
            h[~is_pos] += psi[~is_pos]/a_minus[np.newaxis,:].repeat(psi.shape[0],axis=0)[~is_pos]            
            h[~is_pos] += (theta_minus/a_minus)[np.newaxis,:].repeat(psi.shape[0],axis=0)[~is_pos]
            h[np.isinf(h) | np.isnan(h) | ~tmp] = 0
...
#%%
    ...
    def internal_gradients(self,data_pos,data_neg, l1 = None, l2 = None,weights = None,value='data'):
        gradients = {}
        if self.nature in ['Bernoulli','Spin','Potts']:
            if value == 'data': # data_pos, data_neg are configurations
                ...
                mu_neg = average(data_neg,c=self.n_c)
            elif value == 'mean':
                mu_pos = average(data_pos,weights=weights)
                mu_neg = average(data_neg)
            elif value == 'input':
                mu_pos = average( self.mean_from_inputs(data_pos),weights=weights)
                mu_neg = average( self.mean_from_inputs(data_neg))
            ...
            if value == 'data':
                ...
                mu2_neg = average(self.mean2_from_inputs(data_neg))
                mu_pos = average(self.mean_from_inputs(data_pos),weights=weights)
                mu_neg = average(self.mean_from_inputs(data_neg))                
            ...
            if value == 'data':
                ...
                mu_p_neg = average(np.maximum(data_neg,0))
                mu_n_neg = average(np.minimum(data_neg,0))
            elif value == 'mean':
                ...
                mu_p_neg = average(mu_p_neg)
                mu_n_neg = average(mu_n_neg)
            ...
            if value == 'data':
                ...
                mu2_p_neg = average(np.maximum(data_neg,0)**2)
                mu2_n_neg = average(np.minimum(data_neg,0)**2)
                mu_p_pos = average(np.maximum(data_pos,0),weights=weights)
                mu_n_pos = average(np.minimum(data_pos,0),weights=weights)
                mu_p_neg = average(np.maximum(data_neg,0))
                mu_n_neg = average(np.minimum(data_neg,0))
                ...
                mu_p_neg = average(mu_p_neg)
                mu_n_neg = average(mu_n_neg)                
                mu2_p_neg = average(mu2_p_neg)
                mu2_n_neg = average(mu2_n_neg)
            ...
            if self.position == 'hidden':
                tmp = (gradients['a_plus'] + gradients['a_minus'])/2
                gradients['a_plus'] -= tmp
                gradients['a_minus'] -= tmp
            ...
            if self.position =='visible': # don't need to update a if it is a hidden layer.
                gradients['a'] = -0.5 * (mu2_pos - mu2_neg)
                if weights is not None:
                    gradients['a'] *= weights.sum()/data_pos.shape[0]
            ...
            if value =='data':
                ...
                comu_neg = average_product(data_neg, data_neg)
            ...
            if value == 'data':
                ...
                comu_neg = average_product(data_neg, data_neg,c1=self.n_c, c2= self.n_c)
            elif value == 'mean':
                mu_pos = data_pos[0]
                mu_neg = average(data_neg,c=self.n_c)
                comu_pos = data_pos[1]
        ...
        if (self.position== 'hidden'):
            ...
            # elif self.nature == 'ReLU':
            #     (mu_p_neg0,mu_n_neg0) = self.mean_pm_from_inputs(np.zeros([1,self.N]), beta = 0)
